...
@api_view(['POST'])
@permission_classes([IsAuthenticated])
def create_user_defined_topic(request):  # localhost/botchat/chat/customtopic/
    # 接收前端的数据
    data = request.data
    # 验证用户身份
    ...
    # 根据除了predefined_conversations以外的数据创建新的Topic(将除predefined_conversations和topic_theme之外的信息拼接后作为Topic的custom_context字段,将topic_theme作为Topic的theme字段)
    # 创建Topic
    custom_context_items = [
        data['conversation_time'],
        data['conversation_location'],
        data['conversation_scene'],
        data['instructions'],
    ...
    for key, value in user_data.items():
        if value and key not in ['role', 'personality']:
            custom_context_items.append(f"user_{key}: {value}")
    bot_data = data['bot']
    for key, value in bot_data.items():
        if value and key not in ['role', 'personality']:
            custom_context_items.append(f"bot_{key}: {value}")
    custom_context = '; '.join(custom_context_items)
    new_topic = Topic(
        user=user,  # Assuming the authenticated user is the one creating the topic
